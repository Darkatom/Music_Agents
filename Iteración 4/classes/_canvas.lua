...
function Canvas:newAgent( groupID, x, y )
        ...
        if colliding == nil and x >= 0 and x <= _window_width and y >= 0 and y <= _window_height then
...
function Canvas:destroyMessages()
         ...
         for i = 1, #self.Messages do
             msg = self.Messages[i]
             if self.Agents[msg:getDestinationIndex()]:hasCollided_Instrument( msg:getXPos() + _messages_diam/2, msg:getYPos() + _messages_diam/2 ) then
         ...
         for i = 1, #self.DestroyMessages do
             ...
             if pos <= #self.Messages then
                ...
                msg.playVolume = self.Agents[ self.Messages[pos]:getDestinationIndex() ]:getVolumen()
...
function Canvas:AddVolumeTo( index, addVolume )
         self.Agents[index]:AddVolume( addVolume )
end

function Canvas:AddPitchMinTo( index, addPitch )
         self.Agents[index]:AddPitchMin( addPitch )
end

function Canvas:AddPitchMaxTo( index, addPitch )
         self.Agents[index]:AddPitchMax( addPitch )
end

function Canvas:NextMessageTypeOf( index )
         self.Agents[index]:NextMessageType()
end

function Canvas:PreviousMessageTypeOf( index )
         self.Agents[index]:PreviousMessageType()
end